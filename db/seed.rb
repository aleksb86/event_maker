...
EVENT_PATTERNS = {
  'systemd\[\d+\]:' => { message: ':([a-zA-Z\s]+)' },
  'wpa_supplicant\[\d+\]:' => { interface: 'wlp\ds\d', wpa: 'WPA:\s+([a-zA-Z\s\d:]+)' }
}.freeze
...
EVENT_PATTERNS.each do |key, val|
  EventRule.find_or_create(pattern: key)
  id = EventRule[pattern: key][:id]
  val.each { |name, pattern| AttributeRule.create(event_rule_id: id, name: name, pattern: pattern) }
end
